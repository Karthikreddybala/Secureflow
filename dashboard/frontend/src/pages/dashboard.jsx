...
const getSeverityClass = (severity, attackType) => {
  if (String(attackType || '').toLowerCase() === 'normal') return 'severity-normal'
...
function Dashboard() {
  const allAlerts = useSelector((state) => state.alerts.alerts)
        ...
        <div className="dashboard-col-2">
          <div className="dashboard-row dashboard-row-3">
            <div className="alerts-header">
              <h3>Live Alerts</h3>
              <span className="panel-pill">{recentAlerts.length} visible</span>
            </div>

            {recentAlerts.length === 0 ? (
              <div className="panel-empty">Waiting for WebSocket alerts...</div>
            ) : (
                ...
                {recentAlerts.map((alert, index) => (
                  <div key={alert.id || index} className="alert-item">
                    <div className="alert-item-header">
                      ...
                      <span className={`severity-pill ${getSeverityClass(alert.final?.severity, alert.final?.attack_type)}`}>
                        {isNormalAlert(alert) ? 'Normal' : (alert.final?.severity || 'Unknown')}
                      </span>
                    </div>
